...
<template:addResources type="css" resources="dashboardListings.css,files.css"/>
...
<script type="text/javascript">
    ...
    var myFilesEmptyUploadError = "<fmt:message key="mySettings.errors.picture.empty.upload"/>";
...
<c:choose>
    <c:when test="${not empty param['view']}">
        ...
        <c:if test="${displayView eq 'icon'}">
            <%@include file="myFilesDashboard.settingsDashboard.folder-icon.jspf" %>
        </c:if>
        <c:if test="${displayView eq 'slider'}">
            <%@include file="myFilesDashboard.settingsDashboard.folder-slider.jspf" %>
        </c:if>
    </c:when>
    <c:otherwise>
        <c:set var="displayView" value=""/>
        <%@include file="myFilesDashboard.settingsDashboard.folder.jspf" %>
    </c:otherwise>
</c:choose>
